...
public class Main1 {
    public static void main(String[] args) {
           ...
           for (Tuple2<Integer, Integer> funkwegPaar : funkwegePaare) {
               ...
               if (y1 < y2) { // Antenne 1 oben
                   if (x1 < x2) { // Antenne 1 links oben
                       for (int y = y1 - abstandY, x = x1 - abstandX; (y >= 0 && y < inputList.size()) && (x >= 0 && x < inputList.getFirst().length()); y -= abstandY, x -= abstandX)
                           setzeSchwingungsbauch(inputList, y, x);
                       for (int y = y2 + abstandY, x = x2 + abstandX; (y >= 0 && y < inputList.size()) && (x >= 0 && x < inputList.getFirst().length()); y += abstandY, x += abstandX)
                           setzeSchwingungsbauch(inputList, y, x);
                       ...
                       for (int y = y1 - abstandY, x = x1; (y >= 0 && y < inputList.size()); y -= abstandY)
                           setzeSchwingungsbauch(inputList, y, x);
                       for (int y = y2 + abstandY, x = x2; (y >= 0 && y < inputList.size()); y += abstandY)
                           setzeSchwingungsbauch(inputList, y, x);
                   }
                   ...
                   if (x1 < x2) { // Antenne 1 links gleiche höhe
                       for (int y = y1, x = x1 - abstandX; (x >= 0 && x < inputList.getFirst().length()); x -= abstandX)
                           setzeSchwingungsbauch(inputList, y, x);
                       for (int y = y2, x = x2 + abstandX; (x >= 0 && x < inputList.getFirst().length()); x += abstandX)
                           setzeSchwingungsbauch(inputList, y, x);
        ...
        System.out.println("SchwingungsbauchCount: " + count);
    }
